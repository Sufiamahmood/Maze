...
try:
    character_images = [
        pygame.image.load("Character01.png"),
        pygame.image.load("Character02.png"),
        pygame.image.load("Character03.png"),
        pygame.image.load("Character04.png")
    ]
    for i in range(len(character_images)):
        # Scale character images to fit within the maze cells
        character_images[i] = pygame.transform.scale(character_images[i], (cell_size - 6, cell_size - 6))  # Adjust size here
...
def select_character():
    ...
    character_marker_x = (width // 2) - cell_size * 2 - 20 + (selected_character % 2) * (cell_size * 2 + character_spacing)
    character_marker_y = (height // 2) - cell_size * 2 - 20 + (selected_character // 2) * (cell_size * 2 + character_spacing)
...
def display_character_selection():
    ...
    for i, image in enumerate(character_images):
        x = (width // 2) - cell_size * 2 - 20 + (i % 2) * (cell_size * 2 + character_spacing)
        y = (height // 2) - cell_size * 2 - 20 + (i // 2) * (cell_size * 2 + character_spacing)
        screen.blit(image, (x, y))

    # Display character selection marker
    pygame.draw.rect(screen, BLUE, pygame.Rect(character_marker_x, character_marker_y, cell_size - 6, cell_size - 6), 2)
